...
def validate(payload: Any, not_before: datetime) -> list[str]:
    ...
    for vmid in map(str, range(101, 110)):
        ...
        if compatible is not True or version != "0.4.1" or protocol != 1:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate fresh Hubinet Ops 0.4.2 rollout telemetry"
    )
    ...
    try:
        ...
        print(
            f"Fresh 0.4.2 telemetry validation failed:\n"
    ...
    if errors:
        print(
            "Fresh 0.4.2 telemetry validation failed:\n" + "\n".join(errors),
            file=sys.stderr,
        )
        return 1
    return 0
