...
Before step 1, provision root-owned `/etc/hubinet-ops/hostd.json` and `/etc/hubinet-ops/hostd.env` on PVE. The environment file holds `HUBINET_OPS_HOSTD_TOKEN` and is never committed or printed. The installer migrates the existing CT110 configuration in place: it preserves inventory, addresses, MQTT, monitoring, and health profiles while applying the reviewed 0.4.0 capabilities/hashes and host-control URL.
