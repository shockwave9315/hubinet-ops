...
Hubinet Ops 0.4.3 has two reviewed transactional steps:

1. `deploy/upgrade-0.4.3-from-pve.sh` performs the final manual 0.4.2 → 0.4.3 bootstrap, including PVE host control, hostd, CT110 supervisors, release helpers, agent/config migration, and read-only validation while preserving schema version 400 and durable state.
2. `deploy/install-ha-0.4.3-from-pve.sh [--restart-core] HA_HOST [PORT]` updates the Home Assistant package and generated dashboard.
...
Final validation is read-only: hostd `/health`, wrapper `inspect 100`, `inspect 106`, `list-snapshots 106`, CT110 `/health`, canonical `/api/v1/state`, exact fresh inventory 100–110, executor compatibility CT101–CT109, DB schema version 400, and backend snapshot list. It also requires `hubinet-ops.service` to be active, verifies that the installed CT110 profile URL exactly matches the migrated `api.port`, and runs `/usr/local/sbin/hubinet-maint healthcheck` inside CT110. No update, plan approval, lifecycle, snapshot mutation, repair, or rollback is invoked.

The HA transaction validates every required secret at once, rejects legacy `/api/v1/plans/approve` and `/api/v1/plans/reject` URLs, backs up `configuration.yaml`, `secrets.yaml`, package, and dashboard, stages files, runs `ha core check`, and restores them on failure. By default it does not restart Home Assistant and prints the exact restart command. `--restart-core` restarts Core only after a successful check and waits for Core state `running`; it never changes VM100 lifecycle through PVE. See [the exact 0.4.3 rollout](upgrade-0.4.3.md).
# 0.4.3 bootstrap and later releases

Use `deploy/upgrade-0.4.3-from-pve.sh` only for the final manual 0.4.2 → 0.4.3 bootstrap, followed by `deploy/install-ha-0.4.3-from-pve.sh`. Backups and rollback are documented in `docs/upgrade-0.4.3.md`. From 0.4.4, merge to `main` publishes only after successful CI and a stable version bump; Home Assistant then discovers and explicitly approves the immutable release.
