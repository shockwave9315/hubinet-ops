...
0.4.0 adds scalar discovery entities for executor version/protocol/compatibility/hashes/missing actions/check time, operator capabilities, operation type, lifecycle timing, and snapshot summary. MQTT publishes only `snapshot_count`, latest snapshot name/time/kind, and snapshot operation status—not the complete snapshot list or full job log.
