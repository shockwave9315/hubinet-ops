...
List entries contain `name`, `description`, `created_at`, `kind`, `owned_by_hubinet_ops`, `rollback_eligible`, `delete_eligible`, and `source_job_id`. Normal explicit restore requires `manual_snapshot_restore_allowed`, `snapshot_rollback`, an existing rollback-eligible Hubinet-owned snapshot, no waiting/approved plan, no active global destructive job, and an atomic local job insert before the first hostd POST. Delete rejects names not owned by Hubinet Ops. `latest` is selected by parsed creation time, not lexical accident.
