...
The dashboard contains Centrum plus VM/CT 100–110 views. The main interface uses responsive Sections and Mushroom cards. VM100 remains observation-only for updates and lifecycle, with a narrow exception for typed Hubinet-owned QEMU snapshot create/list/delete. Its RAM-state toggle defaults off; VM100 restore is not exposed. CT101–CT109 expose the complete guarded lifecycle, refresh/scan/plan, healthcheck, and snapshot control set. Normal CT110 shutdown/reboot/force-stop, snapshot create/restore/delete, and self-update call the backend so its SQLite gates remain authoritative.
