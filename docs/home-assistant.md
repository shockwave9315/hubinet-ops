...
Approve/reject sends only VMID to `approve-active`/`reject-active`; it never reads a plan UUID from health attributes. Every action has confirmation and reports success or an explicit backend error through a persistent notification. Force stop, explicit snapshot restore, and snapshot deletion use red destructive styling and explain the consequence. CT110 restore calls the PVE `/snapshots/{name}/restore` route and remains available while the backend guest is offline; it never calls the legacy manual update-rollback endpoint.
