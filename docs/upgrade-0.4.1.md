# Hubinet Ops 0.4.1 production hotfix rollout and rollback

This is an explicit operator-run procedure for an existing 0.3.2 or 0.4.0 installation. Repository tests use stubs and never execute it. Version 0.4.1 does not change the SQLite schema: `PRAGMA user_version` remains `400`.

## Preconditions

1. Review the exact 0.4.1 commit and `CHANGELOG.md`.
...
```text
/root/hubinet-ops-backups/<UTC>-before-0.4.1
```
...
The HA installer stores its backup under `/config/backups/hubinet-ops/<UTC>-before-0.4.1`. Its built-in rollback restores `secrets.yaml`, `configuration.yaml`, the package, and dashboard, then runs `ha core check`. If a Core restart was requested, review the restored configuration before a separate restart.
