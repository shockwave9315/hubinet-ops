...
for vmid in range(100,111):
    ...
    if 101 <= vmid <= 109: state.update(executor_compatible=True,executor_version="0.4.3",executor_protocol_version=1)
    resources[str(vmid)]=state
print(json.dumps({"version":"0.4.3","resources":resources}))
PY
      elif grep -q '/api/v1/resources/106/snapshots' <<<"$script"; then
        echo '{"snapshots":[],"latest":null}'
      elif grep -q 'hubinet-ops-0.4.3.tgz' <<<"$script"; then
...
for argument in "$@"; do
  if [[ "$argument" == Authorization:* ]]; then
    printf ' [redacted authorization]' >> "$TEST_LOG"
  else
    printf ' %s' "$argument" >> "$TEST_LOG"
  fi
done
printf '\n' >> "$TEST_LOG"
if [[ "$*" == *"/api/v1/resources/110/application-release"* ]]; then
  echo '{"status":"up_to_date","current_version":"0.4.3","latest_version":"0.4.3"}'
  exit 0
fi
if [[ -f "$TEST_PVE_ROOT/usr/local/lib/hubinet-ops/hubinet_ops_hostd.py" ]] &&
   grep -Fq 'VERSION = "0.4.3"' "$TEST_PVE_ROOT/usr/local/lib/hubinet-ops/hubinet_ops_hostd.py"; then
  echo '{"status":"ok","version":"0.4.3"}'
else
  echo '{"status":"ok","version":"0.4.2"}'
fi
...
case "$SSH_ORIGINAL_COMMAND" in
  ...
  'list-snapshots 100') echo '{"ok":true,"data":{"snapshots":[]}}' ;;
  'list-snapshots 106') echo '{"ok":true,"data":{"snapshots":[]}}' ;;
  'ct110-system-scan 110') echo '{"ok":true,"data":{"pending_count":0,"packages":[],"fingerprint":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","scanned_at":"2026-08-02T12:00:00+00:00","security_updates_count":0,"reboot_required":false}}' ;;
  *) exit 1 ;;
esac
SH
  cat > "$bin/cp" <<'SH'
...
run_case() {
  ...
  HUBINET_OPS_VALIDATION_DELAY=0 \
    "$REAL_BASH" ${HUBINET_OPS_TEST_BASH_X:+-x} "$ROOT/deploy/upgrade-0.4.3-from-pve.sh" >"$case_root/stdout" 2>"$case_root/stderr"
  rc=$?
  set -e
  printf '%s' "$rc"
}
...
"$REAL_PYTHON" \
  ...
  "$ROOT/deploy/upgrade-0.4.3-from-pve.sh"
...
if grep -Fq '0.4.3 upgrade failed' "$TMP/retry/stderr"; then
  echo "global ERR trap fired before the rc=129 retry succeeded" >&2
  exit 1
fi
grep -Fq 'VERSION = "0.4.3"' "$TMP/success/ct/ct101/usr/local/sbin/hubinet-maint"
grep -Fq 'VERSION = "0.4.3"' "$TMP/success/ct/ct109/usr/local/sbin/hubinet-maint"
grep -Fq 'VERSION = "0.4.3"' "$TMP/success/ct/ct110/usr/local/sbin/hubinet-maint"
grep -Fq 'AGENT_INSTALLED' "$TMP/success/actions.log"
grep -Fq 'AGENT_SSH_PERMISSIONS_NORMALIZED' "$TMP/success/actions.log"
grep -Fq 'VERSION = "0.4.3"' "$TMP/success/pve/usr/local/lib/hubinet-ops/hubinet_ops_hostd.py"
...
echo "0.4.3 runtime smoke: success, retry and cross-layer rollback passed"
