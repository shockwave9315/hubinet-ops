...
for vmid in range(100,111):
    ...
    if 101 <= vmid <= 109: state.update(executor_compatible=True,executor_version="0.4.0",executor_protocol_version=1)
    resources[str(vmid)]=state
print(json.dumps({"version":"0.4.0","resources":resources}))
PY
      elif grep -q '/api/v1/resources/106/snapshots' <<<"$script"; then
        echo '{"snapshots":[],"latest":null}'
      elif grep -q 'hubinet-ops-0.4.0.tgz' <<<"$script"; then
...
run_case() {
  local name="$1" fail_vmid="${2:-}" case_root
  case_root="$TMP/$name"
  mkdir -p "$case_root"
  ...
  HUBINET_OPS_VALIDATION_DELAY=0 \
    bash "$ROOT/deploy/upgrade-0.4.0-from-pve.sh" >"$case_root/stdout" 2>"$case_root/stderr"
  rc=$?
  set -e
  printf '%s' "$rc"
}
...
grep -Fq 'VERSION = "0.4.0"' "$TMP/success/ct/ct101/usr/local/sbin/hubinet-maint"
...
echo "0.4.0 runtime smoke: success and cross-layer rollback passed"
