...
fail() {
  echo "0.4.2 HA installer runtime smoke failed: $*" >&2
  exit 1
}
...
"$REAL_PYTHON" \
  ...
  "$ROOT/deploy/install-ha-0.4.2-from-pve.sh"
...
set -Eeuo pipefail
  install -d -m 0700 '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2'
  install -d -m 0755 /config/packages /config/dashboards
  cp -a /config/secrets.yaml '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/secrets.yaml'
  cp -a /config/configuration.yaml '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/configuration.yaml'
  [[ ! -f /config/packages/hubinet_ops.yaml ]] || cp -a /config/packages/hubinet_ops.yaml '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.package.yaml'
  [[ ! -f /config/dashboards/hubinet_ops.yaml ]] || cp -a /config/dashboards/hubinet_ops.yaml '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.dashboard.yaml'
  : > '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/backup.complete'
EOF
...
set -Eeuo pipefail
      cp -a '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/secrets.yaml' /config/secrets.yaml
      cp -a '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/configuration.yaml' /config/configuration.yaml
      if [[ -f '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.package.yaml' ]]; then
        cp -a '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.package.yaml' /config/packages/hubinet_ops.yaml
      else
        rm -f /config/packages/hubinet_ops.yaml
      fi
      if [[ -f '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.dashboard.yaml' ]]; then
        cp -a '/config/backups/hubinet-ops/20260102T030405Z-before-0.4.2/hubinet_ops.dashboard.yaml' /config/dashboards/hubinet_ops.yaml
      else
...
run_case() {
  ...
  (
    ...
    "$REAL_ENV" -i \
      ...
      "$ROOT/deploy/install-ha-0.4.2-from-pve.sh" "${args[@]}"
...
echo "0.4.2 HA installer runtime smoke: passed"
