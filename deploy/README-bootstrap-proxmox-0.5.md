...
> >=24-hour observation window completed, while the optional/synthetic §6/§7
> scenarios remain unexercised. The current decision is GO WITH OBSERVATION.
...
- **Third real dogfood** (fresh run from commit
  ...
  recommended >=24-hour observation window. The optional/synthetic
  operational scenarios remain unexercised, so the current decision remains
  GO WITH OBSERVATION.
