...
[[ $# -eq 0 ]] || { echo "The 0.4.3 upgrade accepts no arguments" >&2; exit 2; }

SOURCE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_ROOT="${HUBINET_OPS_BACKUP_ROOT:-/root/hubinet-ops-backups}"
BACKUP="$BACKUP_ROOT/${STAMP}-before-0.4.3"
PVE_ROOT="${HUBINET_OPS_TEST_PVE_ROOT:-}"
AGENT_VMID=110
AGENT_BACKUP="$BACKUP/ct110"
ARCHIVE="${HUBINET_OPS_TEST_ARCHIVE:-/tmp/hubinet-ops-0.4.3-${STAMP}.tgz}"
...
HOST_DESTINATIONS=(
  ...
  /usr/local/lib/hubinet-ops/hubinet_ops_ct110_system.py
  ...
  /etc/hubinet-ops/ct110-system-automatic-rollback-vmids
  "$HOSTD_ENV"
)
...
required_source=(
  ...
  deploy/pve/hubinet_ops_ct110_system.py
  ...
  deploy/pve/ct110-system-update-vmids
  deploy/pve/ct110-system-automatic-rollback-vmids
...
grep -Fq 'VERSION = "0.4.3"' "$SOURCE_DIR/app/mqtt.py"
grep -Fq 'VERSION = "0.4.3"' "$SOURCE_DIR/deploy/managed/hubinet-maint"
...
python3 -m py_compile \
  ...
  "$SOURCE_DIR/deploy/pve/hubinet_ops_ct110_system.py"
bash -n "$SOURCE_DIR/deploy/pve/hubinet-ops-host" \
  "$SOURCE_DIR/deploy/pve/hubinet-ops-self-update" \
  "$SOURCE_DIR/deploy/pve/hubinet-ops-ct110-system-update"
python3 -m json.tool "$(pve_path "$HOSTD_CONFIG")" >/dev/null
HOSTD_DATABASE="$(python3 - "$(pve_path "$HOSTD_CONFIG")" <<'PY'
import json, pathlib, sys
value=str(json.load(open(sys.argv[1], encoding="utf-8"))["database"])
path=pathlib.PurePosixPath(value)
if not path.is_absolute() or value == "/" or any(part == ".." for part in path.parts):
    raise SystemExit("hostd database path is unsafe")
print(value)
PY
)"
HOST_DESTINATIONS+=("$HOSTD_DATABASE")
...
if data.get("version") != "0.4.2":
    raise SystemExit("The 0.4.3 upgrade supports only an installed 0.4.2 backend")
PY
installed_hostd_health="$(curl -fsS --max-time 5 "${HOST_CONTROL_URL%/}/health")"
python3 - "$installed_hostd_health" <<'PY'
import json, sys
data=json.loads(sys.argv[1])
if data.get("status") != "ok" or data.get("version") != "0.4.2":
    raise SystemExit("The 0.4.3 upgrade supports only an installed 0.4.2 hostd")
PY
...
if data.get("version") != "0.4.3" or data.get("protocol_version") != 1: raise SystemExit("incompatible executor version/protocol")
...
rollback_all() {
  ...
  echo "0.4.3 upgrade failed; restoring all modified layers" >&2
...
if [[ -f "$(pve_path "$HOSTD_DATABASE")" ]]; then
  python3 - "$(pve_path "$HOSTD_DATABASE")" <<'PY'
import sqlite3, sys
with sqlite3.connect(f"file:{sys.argv[1]}?mode=ro", uri=True) as db:
    count=db.execute("SELECT count(*) FROM host_jobs WHERE status IN ('queued','running')").fetchone()[0]
if count:
    raise SystemExit("Active durable host job blocks the 0.4.3 bootstrap")
PY
fi
systemctl stop "$HOSTD_SERVICE"
...
python3 "$SOURCE_DIR/scripts/validate_pve_snapshot_policy.py" \
  ...
  --policy-dir "$SOURCE_DIR/deploy/pve"
PYTHONPATH="$SOURCE_DIR" python3 - "$BACKUP/agent-config-0.4.3.yaml" <<'PY'
import sys, yaml
...
pct_retry_129 push "$AGENT_VMID" "$BACKUP/agent-config-0.4.3.yaml" /etc/hubinet-ops/config.yaml.new --perms 0640
...
install -m 0644 "$SOURCE_DIR/deploy/pve/hubinet_ops_ct110_system.py" "$(pve_path /usr/local/lib/hubinet-ops/hubinet_ops_ct110_system.py)"
...
install -m 0755 "$SOURCE_DIR/deploy/pve/hubinet-ops-ct110-system-update" "$(pve_path /usr/local/sbin/hubinet-ops-ct110-system-update)"
...
for name in observation-vmids managed-vmids maintenance-vmids lifecycle-vmids \
  ...
  ct110-system-automatic-rollback-vmids; do
...
python3 -m py_compile \
  ...
  "$(pve_path /usr/local/lib/hubinet-ops/hubinet_ops_ct110_system.py)"
bash -n "$(pve_path /usr/local/sbin/hubinet-ops-host)" \
  "$(pve_path /usr/local/sbin/hubinet-ops-self-update)" \
  "$(pve_path /usr/local/sbin/hubinet-ops-ct110-system-update)"
...
qemu_snapshot_smoke="$(SSH_ORIGINAL_COMMAND='list-snapshots 100' "$wrapper")"
ct110_system_scan="$(SSH_ORIGINAL_COMMAND='ct110-system-scan 110' "$wrapper")"
python3 - "$qemu_smoke" "$lxc_smoke" "$snapshot_smoke" "$qemu_snapshot_smoke" "$ct110_system_scan" <<'PY'
import json, math, sys
qemu, lxc, snapshots, qemu_snapshots, system_scan = [json.loads(value) for value in sys.argv[1:]]
...
if qemu_snapshots.get("ok") is not True or not isinstance(qemu_snapshots.get("data", {}).get("snapshots"), list): raise SystemExit("invalid read-only QEMU snapshot list")
scan_data=system_scan.get("data", {})
if system_scan.get("ok") is not True or not isinstance(scan_data.get("packages"), list): raise SystemExit("invalid read-only CT110 system scan")
PY
PYTHONPATH="$SOURCE_DIR/deploy/pve" python3 - <<'PY'
from hubinet_ops_host_control import _snapshot_description_metadata, parse_snapshot
parsed=parse_snapshot("hubinet-ops-109-pre-20260801T212708Z", 109)
metadata=_snapshot_description_metadata(
    "hubinet-ops;kind=pre-update;created_at=2026-08-01T21:27:08+00:00;source_job_id=3a337ae2263c4b57aa5e0e8d7a0fae51\n",
    parsed,
)
if metadata is None or metadata.get("source_job_id") != "3a337ae2263c4b57aa5e0e8d7a0fae51":
    raise SystemExit("PVE newline snapshot fixture failed")
PY
...
import json, sys
data=json.loads(sys.argv[1])
if data.get("status") != "ok" or data.get("version") != "0.4.3": raise SystemExit("hostd health failed")
PY
release_check="$(curl -fsS --max-time 30 \
  -H "Authorization: Bearer $HUBINET_OPS_HOSTD_BACKEND_TOKEN" \
  "${HOST_CONTROL_URL%/}/api/v1/resources/110/application-release")"
python3 - "$release_check" <<'PY'
import json, sys
data=json.loads(sys.argv[1])
if data.get("status") not in {"up_to_date", "update_available", "no_release_published"}:
    raise SystemExit("application release discovery endpoint failed")
PY

tar -C "$SOURCE_DIR" -czf "$ARCHIVE" app
pct_retry_129 push "$AGENT_VMID" "$ARCHIVE" /root/hubinet-ops-0.4.3.tgz --perms 0600
agent_changes_started=true
VALIDATION_NOT_BEFORE="$(date -u +%Y-%m-%dT%H:%M:%S+00:00)"
pct exec "$AGENT_VMID" -- bash -s <<'REMOTE_INSTALL_AGENT'
set -Eeuo pipefail
staging=/root/hubinet-ops-0.4.3
rm -rf "$staging"
install -d -m 0700 "$staging"
tar -xzf /root/hubinet-ops-0.4.3.tgz -C "$staging"
grep -Fq 'VERSION = "0.4.3"' "$staging/app/mqtt.py"
...
rm -rf "$staging" /root/hubinet-ops-0.4.3.tgz
...
REMOTE_READ_STATE
    ...
    if printf '%s' "$states" | \
      python3 "$SOURCE_DIR/scripts/validate_rollout_state_0_4_3.py" \
        "$VALIDATION_NOT_BEFORE"
    then
      validation_ok=true
      break
