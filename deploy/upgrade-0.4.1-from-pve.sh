...
[[ $# -eq 0 ]] || { echo "The 0.4.1 upgrade accepts no arguments" >&2; exit 2; }

SOURCE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_ROOT="${HUBINET_OPS_BACKUP_ROOT:-/root/hubinet-ops-backups}"
BACKUP="$BACKUP_ROOT/${STAMP}-before-0.4.1"
PVE_ROOT="${HUBINET_OPS_TEST_PVE_ROOT:-}"
AGENT_VMID=110
AGENT_BACKUP="$BACKUP/ct110"
ARCHIVE="${HUBINET_OPS_TEST_ARCHIVE:-/tmp/hubinet-ops-0.4.1-${STAMP}.tgz}"
...
required_source=(
  ...
  scripts/migrate_config_0_4_1.py
  scripts/validate_rollout_state_0_4_1.py
)
...
grep -Fq 'VERSION = "0.4.1"' "$SOURCE_DIR/app/mqtt.py"
...
python3 "$SOURCE_DIR/scripts/migrate_config_0_4_1.py" \
  "$BACKUP/agent-config.yaml" "$BACKUP/agent-config-0.4.1.yaml" \
  --host-control-url "$HOST_CONTROL_URL"
python3 "$SOURCE_DIR/scripts/validate_pve_snapshot_policy.py" \
  "$BACKUP/agent-config-0.4.1.yaml" \
  --policy-dir "$SOURCE_DIR/deploy/pve"
set -a
...
pct_retry_129 push "$AGENT_VMID" "$BACKUP/agent-config-0.4.1.yaml" /etc/hubinet-ops/config.yaml.new --perms 0640
...
if data.get("status") != "ok" or data.get("version") != "0.4.1": raise SystemExit("hostd health failed")
PY

tar -C "$SOURCE_DIR" -czf "$ARCHIVE" app
pct_retry_129 push "$AGENT_VMID" "$ARCHIVE" /root/hubinet-ops-0.4.1.tgz --perms 0600
agent_changes_started=true
VALIDATION_NOT_BEFORE="$(date -u +%Y-%m-%dT%H:%M:%S+00:00)"
pct exec "$AGENT_VMID" -- bash -s <<'REMOTE_INSTALL_AGENT'
set -Eeuo pipefail
staging=/root/hubinet-ops-0.4.1
rm -rf "$staging"
install -d -m 0700 "$staging"
tar -xzf /root/hubinet-ops-0.4.1.tgz -C "$staging"
grep -Fq 'VERSION = "0.4.1"' "$staging/app/mqtt.py"
...
rm -rf "$staging" /root/hubinet-ops-0.4.1.tgz
...
REMOTE_READ_STATE
    ...
    if printf '%s' "$states" | \
      python3 "$SOURCE_DIR/scripts/validate_rollout_state_0_4_1.py" \
        "$VALIDATION_NOT_BEFORE"
    then
      validation_ok=true
      break
...
[[ "$validation_ok" == true ]] || { echo "Fresh 0.4.1 telemetry validation failed after all attempts" >&2; rollback_all 1; }
...
trap - ERR INT TERM EXIT
rm -f "$ARCHIVE"
echo "Hubinet Ops 0.4.1 installed transactionally. Backup: $BACKUP"
