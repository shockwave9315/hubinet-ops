...
BACKUP_DIR="/config/backups/hubinet-ops/${STAMP}-before-0.4.2"
...
python3 -m py_compile "$SOURCE_DIR/scripts/validate_ha_secrets_0_4_2.py"
...
ssh "${SSH_ARGS[@]}" 'cat /config/secrets.yaml' |
  python3 "$SOURCE_DIR/scripts/validate_ha_secrets_0_4_2.py" -
...
trap - ERR INT TERM EXIT
echo "Hubinet Ops 0.4.2 HA files validated. Backup: $HA_HOST:$BACKUP_DIR"
if [[ "$restart_core" == true ]]; then
