...
BACKUP_DIR="/config/backups/hubinet-ops/${STAMP}-before-0.3.0"
URL_FILE="/tmp/hubinet-ops-0.3.0-urls-${STAMP}"
...
cat > "$URL_FILE" <<EOF
hubinet_ops_scan_url: "$AGENT_BASE_URL/api/v1/resources/{{ vmid }}/scan"
hubinet_ops_refresh_url: "$AGENT_BASE_URL/api/v1/resources/{{ vmid }}/refresh"
...
hubinet_ops_start_url: "$AGENT_BASE_URL/api/v1/resources/{{ vmid }}/start"
hubinet_ops_shutdown_url: "$AGENT_BASE_URL/api/v1/resources/{{ vmid }}/shutdown"
hubinet_ops_reboot_url: "$AGENT_BASE_URL/api/v1/resources/{{ vmid }}/reboot"
EOF
scp -P "$HA_PORT" -i /root/.ssh/id_ed25519 "$URL_FILE" \
  "root@$HA_HOST:/tmp/hubinet-ops-0.3.0-urls"
...
ssh "${SSH_ARGS[@]}" '
  ...
  done < /tmp/hubinet-ops-0.3.0-urls
  chmod 600 /config/secrets.yaml
  rm -f /tmp/hubinet-ops-0.3.0-urls
  ha core check
'
...
trap - ERR INT TERM
rm -f "$URL_FILE"
echo "Hubinet Ops 0.3.0 HA files validated. Backup: $HA_HOST:$BACKUP_DIR"
