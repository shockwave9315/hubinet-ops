...
BACKUP_DIR="/config/backups/hubinet-ops/${STAMP}-before-0.3.1"
...
ssh "${SSH_ARGS[@]}" '
  ...
  for key in \
    ...
    grep -q "^$key:" /config/secrets.yaml || {
      echo "Missing required existing 0.3.0 secret: $key" >&2
      exit 1
    }
  done
...
complete=true
trap - ERR INT TERM EXIT
echo "Hubinet Ops 0.3.1 HA files validated. Backup: $HA_HOST:$BACKUP_DIR"
